...
const SearchForm = () => {
  ...
  const [departure, setDeparture] = useState(
    new Date(arivalDate.getTime() + 24 * 60 * 60 * 1000)
  );
  const navigate = useNavigate();
  const handleSubmit = (e) => {
    e.preventDefault();
    console.log(location);
    console.log(arivalDate);
      ...
      <form onSubmit={handleSubmit} className="mt-6">
        <div className="shadow-md rounded-md my-2 p-3">
          ...
          <input
            ...
            className="block w-full mt-1 p-1 text-gray-700 bg-white   focus:border-rose-400 focus:ring-rose-300 focus:outline-none focus:ring focus:ring-opacity-40"
          />
        </div>
        ...
        <div className="flex justify-between">
          <div className="shadow-md rounded-md my-2 p-3 flex justify-between items-center">
            <div>
              ...
              <DatePicker
                selected={arivalDate}
                onChange={(date) => setArivalDate(date)}
                className="w-2/3"
              />
            </div>
